...
public class FileUtil {
	...
	/*获取绝对路径*/
	public static String getRealPath(String productImgPath) {
		String realPath = WebUtil.getRequest().getServletContext().getRealPath(productImgPath);
		return realPath;
	}
	...
	/*springMVC上传图片*/
	public static String uploadImages(MultipartFile Image,String uploadPath) {
		try {
			//获取输入流
			InputStream inputStream = Image.getInputStream();
			//获取文件名
			String filename = Image.getOriginalFilename();
			//获取项目发布的路径   参数是相对与webApp来说的
			String realPath = WebUtil.getRequest().getServletContext().getRealPath(uploadPath);
			String imgPath = FileUtil.copyFile(inputStream, filename, realPath);
